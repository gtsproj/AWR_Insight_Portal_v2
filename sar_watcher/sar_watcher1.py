...
def convert_binary_sar(binary_path: str, hostname: str) -> str | None:
    ...
    output_name = f"{hostname}_{basename}_{datetime.now().strftime('%Y%m%d%H%M%S')}.txt"
...
def already_queued(hostname: str, filepath: str) -> bool:
    ...
    return any(i.get("filepath") == filepath for i in queue)


def enqueue(hostname: str, filepath: str,
            original_file: str = None, file_type: str = "text") -> None:
    """Add a SAR file to the host's queue with mtime-based ordering."""
    queue = load_queue(hostname)
    # Use file modification time as sort key for sequential processing
    try:
        mtime = datetime.fromtimestamp(os.path.getmtime(original_file or filepath))
...
def archive_original(hostname: str, filepath: str) -> None:
    ...
    try:
        ...
        logger.debug(f"Archived original: {os.path.basename(filepath)}")
    except Exception as e:
...
def _process_sar_file(filepath: str, hostname: str, wsl_ok: bool):
    ...
    if is_text_sar(filename):
        if not already_queued(hostname, filepath):
            enqueue(hostname, filepath, file_type="text")
            archive_original(hostname, filepath)
        ...
        if not wsl_ok:
            ...
            return
        converted = convert_binary_sar(filepath, hostname)
        if converted:
            if not already_queued(hostname, converted):
                enqueue(hostname, converted,
                        original_file=filepath,
                        file_type="binary_converted")
            archive_original(hostname, filepath)
        else:
            logger.error(f"❌ Binary conversion failed for {filename} — left in place for retry")
